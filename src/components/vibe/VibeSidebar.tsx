    ...
    <div className="h-full flex flex-col">
      <div className="mb-8 shrink-0">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className={`text-[10px] font-black uppercase tracking-[0.4em] ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      <div className="flex-1 overflow-hidden relative">
            ...
            {activities.map((activity) => (
              <motion.div
                ...
                className={`p-6 rounded-[2rem] border ${
                  activity.type === 'spray' 
                    ? 'bg-[#D4AF37]/10 border-[#D4AF37]/30 shadow-[0_0_30px_-10px_rgba(212,175,55,0.3)]' 
              ...
              >
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                    ...
                    {activity.type === 'spray' ? <Coins size={18} /> : 
                     activity.type === 'checkin' ? <UserCheck size={18} /> : <MessageSquare size={18} />}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h4 className={`text-lg font-serif italic truncate ${isDark ? 'text-white' : 'text-black'}`}>
                      {activity.detail}
                    </h4>
                    {activity.amount && (
                      <p className="text-2xl font-serif italic text-[#D4AF37] mt-1">
        ...
        <div className={`absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t ${
          isDark ? 'from-[#050505] to-transparent' : 'from-[#f8f8f8] to-transparent'
        } pointer-events-none`} />
      </div>
    </div>
  );
