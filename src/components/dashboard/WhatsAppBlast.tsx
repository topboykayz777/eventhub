...
import { X, Send, Play, SkipForward, Save, FileText, ArrowRight } from 'lucide-react';
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-6">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-3xl w-full bg-[#0f0f0f] border border-white/10 p-8 md:p-12 rounded-[3rem] shadow-2xl max-h-[90vh] flex flex-col">
              ...
              {!isLooping ? (
                <div className="space-y-10">
                  ...
                  <div className="space-y-4">
                    ...
                    <Textarea placeholder="Compose your message..." className="min-h-[180px] bg-white/5 border-white/10 rounded-none focus:border-[#25D366]/50 text-lg font-light resize-none" value={message} onChange={(e) => setMessage(e.target.value)} />
                  </div>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                      {rsvps.map((rsvp) => (
                        <div key={rsvp.id} onClick={() => setSelectedIds(prev => prev.includes(rsvp.id) ? prev.filter(i => i !== rsvp.id) : [...prev, rsvp.id])} className={`p-4 border cursor-pointer transition-all ${selectedIds.includes(rsvp.id) ? 'bg-[#25D366]/10 border-[#25D366]/30' : 'bg-white/5 border-white/5 hover:border-white/20'}`}>
                          <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${selectedIds.includes(rsvp.id) ? 'text-white' : 'text-gray-500'}`}>{rsvp.guest_name}</p>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center py-12">
                  <div className="w-full bg-white/5 h-1.5 mb-16 rounded-full overflow-hidden">
                    <motion.div className="bg-[#25D366] h-full shadow-[0_0_15px_rgba(37,211,102,0.5)]" initial={{ width: 0 }} animate={{ width: `${((currentIndex + 1) / selectedGuests.length) * 100}%` }} />
                  </div>
            ...
            <div className="pt-10 shrink-0">
              {!isLooping ? (
                <Button onClick={() => { if (!message.trim() || selectedIds.length === 0) return; setIsLooping(true); }} className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white py-10 rounded-none text-[10px] font-bold tracking-[0.4em] uppercase shadow-2xl shadow-[#25D366]/10"><Play className="w-4 h-4 mr-2" /> Initialize Blast Sequence</Button>
              ) : (
