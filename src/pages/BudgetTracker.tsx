...
import { ArrowLeft, Plus, Trash2, TrendingUp, TrendingDown, Wallet, Loader2, Coins, CheckCircle2, XCircle, Clock } from 'lucide-react';
      ...
      <div className="max-w-5xl mx-auto py-24 px-6">
        ...
        <AnimatePresence>
          {pendingSprays.length > 0 && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="mb-16">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          <GlassCard className="p-8 border-white/5" hover={false}>
            <div className="flex items-center gap-4 mb-4"><TrendingUp className="text-green-500 w-4 h-4" /><span className="text-[8px] font-bold uppercase tracking-widest text-gray-500">Total Income</span></div>
            <div className="text-2xl font-serif italic">₦{totalIncome.toLocaleString()}</div>
          </GlassCard>
          <GlassCard className="p-8 border-white/5" hover={false}>
            <div className="flex items-center gap-4 mb-4"><TrendingDown className="text-red-500 w-4 h-4" /><span className="text-[8px] font-bold uppercase tracking-widest text-gray-500">Total Expenses</span></div>
            <div className="text-2xl font-serif italic">₦{totalExpense.toLocaleString()}</div>
          </GlassCard>
        ...
        {/* Approved History */}
        <div className="space-y-4">
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-500 px-8 block mb-6">Transaction History</span>
          {approvedItems.map((item) => (
            <GlassCard key={item.id} className="p-8 border-white/5 group">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-6">
                  <div className={`w-2 h-2 rounded-full ${item.type === 'income' ? 'bg-green-500' : 'bg-red-500'}`} />
                  <div>
                    <p className="text-lg font-light text-white">{item.description}</p>
                    <p className="text-[8px] font-bold uppercase tracking-widest text-gray-600">{new Date(item.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
