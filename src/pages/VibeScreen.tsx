...
import { Coins, UserCheck, Sparkles, Users, QrCode, Clock, Loader2 } from 'lucide-react';
...
const VibeScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden relative font-serif">
      ...
      <div className="relative z-10 h-screen flex flex-col p-12 md:p-20">
        {/* Header */}
        <div className="flex justify-between items-start mb-20">
          <div>
            <span className="text-[#D4AF37] text-xs font-bold tracking-[0.6em] uppercase mb-4 block">Live Event Feed</span>
            <h1 className="text-6xl md:text-8xl italic leading-tight">{event.event_name}</h1>
          </div>
          <div className="text-right">
            <div className="flex items-center gap-4 text-4xl font-light tracking-widest mb-2">
              <Clock className="text-[#D4AF37] w-8 h-8" />
              {currentTime.toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
            </div>
            <p className="text-gray-500 text-sm uppercase tracking-[0.4em]">{currentTime.toLocaleDateString('en-NG', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          </div>
        </div>

        <div className="flex-grow grid grid-cols-12 gap-20">
          {/* Left: Live Feed */}
          <div className="col-span-7 space-y-8">
            <h2 className="text-xs font-bold uppercase tracking-[0.4em] text-gray-500 mb-12 flex items-center gap-4">
                ...
                {activities.length === 0 ? (
                  ...
                  activities.map((activity) => (
                    <motion.div
                      ...
                      className="bg-white/[0.02] border border-white/5 p-10 rounded-[3rem] flex items-center justify-between group hover:bg-white/[0.04] transition-all"
                    >
                      <div className="flex items-center gap-8">
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center ${activity.type === 'spray' ? 'bg-[#D4AF37] text-black' : 'bg-white/10 text-[#D4AF37]'}`}>
                          {activity.type === 'spray' ? <Coins size={32} /> : <UserCheck size={32} />}
                        </div>
                        <div>
                          <p className="text-[#D4AF37] text-[10px] font-bold uppercase tracking-[0.4em] mb-2">{activity.title}</p>
                          <p className="text-4xl font-light italic">{activity.subtitle}</p>
                        </div>
                      </div>
                      {activity.amount && (
          ...
          <div className="col-span-5 space-y-12">
            <div className="grid grid-cols-2 gap-8">
              <div className="bg-white/[0.02] border border-white/5 p-12 rounded-[3rem] text-center">
                <Users className="text-[#D4AF37] w-10 h-10 mx-auto mb-6" />
                <p className="text-6xl font-light mb-2">{stats.rsvps}</p>
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-500">Guests Confirmed</p>
              </div>
              <div className="bg-white/[0.02] border border-white/5 p-12 rounded-[3rem] text-center">
                <Coins className="text-[#D4AF37] w-10 h-10 mx-auto mb-6" />
                <p className="text-4xl font-light mb-2">₦{stats.sprays.toLocaleString()}</p>
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-500">Total Sprayed</p>
              </div>
            </div>

            <div className="bg-[#D4AF37] p-16 rounded-[4rem] text-black text-center relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 -mr-16 -mt-16 rotate-45" />
              <div className="relative z-10">
                <QrCode className="w-12 h-12 mx-auto mb-8 opacity-50" />
                <h3 className="text-4xl font-serif italic mb-4">Join the Celebration</h3>
                <p className="text-xs font-bold uppercase tracking-[0.2em] mb-12 opacity-60">Scan to RSVP or Spray the Host</p>
                
                <div className="bg-white p-8 rounded-[2.5rem] inline-block shadow-2xl transform group-hover:scale-105 transition-transform duration-500">
                  <QRCodeSVG value={eventUrl} size={240} level="H" />
                </div>
                
                <p className="mt-12 text-[10px] font-black uppercase tracking-[0.4em]">eventhub.ng/event/{event.slug}</p>
              </div>
            </div>
          </div>
        </div>
