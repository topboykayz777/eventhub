...
const AdminDashboard = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (passcode.trim() === DEFAULT_PASSCODE || passcode.trim() === "eventhub2026") {
      setIsAuthenticated(true);
        ...
        <div className="max-w-md mx-auto w-full px-6 py-20">
          ...
          >
            ...
            <p className="text-[8px] text-muted-foreground font-mono uppercase tracking-widest">
              Default passcode: <span className="text-[#D4AF37]">2026</span>
            </p>
          </motion.div>
        </div>
                    ...
                    <td className="py-5 pr-4">
                      <div className="flex items-center gap-2">
                        <span className={`text-[8px] font-black uppercase px-2.5 py-1 rounded-full ${
                          ev.is_paid ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20'
                        }`}>
                          {ev.plan || 'beta'}
                        </span>
                        {ev.is_finished && (
