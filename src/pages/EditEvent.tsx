...
import { ArrowLeft, Upload, X, Palette, Sparkles, Calendar, MapPin, Type, Image as ImageIcon, Save, Loader2, Crown, Gem, Star, Heart, Flower2, Waves, Sun, Moon, Landmark, PenTool, Clock, PlayCircle, ZapIcon, Cherry, Trees as PalmTree, Tent, Ghost, Palette as ColorPalette, Camera } from 'lucide-react';
...
const EditEvent = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, isGallery = false) => {
    ...
    if (isGallery) {
      // Beta mirrors Pro limits (50 items)
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen bg-[#0f0f0f]"><div className="w-10 h-10 border-2 border-[#D4AF37] border-t-transparent rounded-full animate-spin" /></div>;
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-white">
      ...
      <div className="max-w-5xl mx-auto py-12 md:py-24 px-4 md:px-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16">
          <Button variant="ghost" onClick={() => navigate('/dashboard')} className="text-gray-400 hover:text-[#D4AF37] p-0"><ArrowLeft className="w-4 h-4 mr-2" /> Dashboard</Button>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          <GlassCard className="p-8 md:p-12 border-white/5">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.3em] text-white mb-10">Event Identity</h2>
            <div className="grid md:grid-cols-2 gap-10">
              <div className="space-y-3"><Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Event Title</Label><Input required className="h-16 bg-white/5 border-white/10 rounded-none focus:border-[#D4AF37]/50 text-lg font-light" value={formData.eventName} onChange={(e) => setFormData({ ...formData, eventName: e.target.value })} /></div>
              <div className="space-y-3"><Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Venue Address</Label><Input required className="h-16 bg-white/5 border-white/10 rounded-none focus:border-[#D4AF37]/50 text-lg font-light" value={formData.venue} onChange={(e) => setFormData({ ...formData, venue: e.target.value })} /></div>
            </div>
          </GlassCard>
          <GlassCard className="p-8 md:p-12 border-white/5">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.3em] text-white mb-10">Visual Assets</h2>
            <div className="space-y-10">
              <div className="space-y-4">
                <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Cover Portrait</Label>
                <div className="flex flex-col sm:flex-row items-center gap-8">
                  {formData.photo_url && <div className="w-full sm:w-32 h-48 sm:h-32 border border-white/10 overflow-hidden">{isVideo(formData.photo_url) ? <video src={formData.photo_url} className="w-full h-full object-cover" muted /> : <img src={formData.photo_url} className="w-full h-full object-cover" alt="Cover" />}</div>}
                  <Label htmlFor="cover-upload" className="cursor-pointer h-32 w-full flex-1 border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-3 hover:border-[#D4AF37]/30 transition-colors bg-white/5">
                    ...
                    <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-gray-500">Change Cover</span>
                    <input id="cover-upload" type="file" className="hidden" onChange={(e) => handleFileUpload(e)} />
                  </Label>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex justify-between items-center"><Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Media Vault ({eventPlan})</Label><span className="text-[8px] font-black uppercase tracking-widest text-[#D4AF37]">{(formData.gallery_urls?.length || 0)} / {(eventPlan === 'Pro' || eventPlan === 'beta') ? '50' : eventPlan === 'Standard' ? '10' : '0'}</span></div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                  {(formData.gallery_urls || []).map((url, i) => (
                    <div key={i} className="relative aspect-square border border-white/10 group overflow-hidden">
                      ...
                      <button type="button" onClick={() => removeGalleryPhoto(url)} className="absolute top-2 right-2 bg-black/50 p-1.5 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500"><X size={14} /></button>
                    </div>
                  ))}
                  {(eventPlan === 'Standard' || eventPlan === 'Pro' || eventPlan === 'beta') && (
                    <Label htmlFor="gallery-upload" className="cursor-pointer aspect-square border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-2 hover:border-[#D4AF37]/30 transition-colors bg-white/5">
                      <Upload className="w-4 h-4 text-gray-600" /><span className="text-[7px] font-bold uppercase tracking-[0.1em] text-gray-500">Add Media</span>
          ...
          <GlassCard className="p-8 md:p-12 border-white/5">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.3em] text-white mb-10">Aesthetic Theme</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
              {themes.map((t) => (
                <button key={t.id} type="button" onClick={() => setFormData({ ...formData, theme: t.id })} className={`relative p-6 border transition-all text-left overflow-hidden h-32 ${formData.theme === t.id ? 'border-[#D4AF37] bg-[#D4AF37]/5' : 'border-white/5 hover:border-white/10'}`}>
                  <div className="relative z-10 flex flex-col justify-between h-full"><t.icon className={`w-5 h-5 ${formData.theme === t.id ? 'text-[#D4AF37]' : 'text-gray-600'}`} /><span className="text-[8px] font-bold uppercase tracking-widest">{t.label}</span></div>
                  <div className={`absolute top-0 right-0 w-12 h-12 ${t.color} opacity-20 -mr-6 -mt-6 rotate-45`} />
                </button>
              ))}
            </div>
          </GlassCard>
          <Button type="submit" disabled={saving || uploading} className="w-full bg-[#D4AF37] hover:bg-[#B8860B] text-black py-10 rounded-none text-[10px] font-bold tracking-[0.4em] uppercase transition-all duration-500">
            {saving ? 'Refining...' : 'Save Changes'} <Save className="ml-2 w-4 h-4" />
          </Button>
        </form>
      </div>
    </div>
